...
class NOPPERS_OT_SaveTemplate(Operator):
    ...
    def execute(self, context):
        ...
        if is_new:
            ...
            name = scene.export_template_name.strip()
            if not name:
            ...
            if not selected:
                ...
                return {'CANCELLED'}
            pairs = [f"{obj.name}:{obj.export_target_name or obj.name}" for obj in selected]
        else:
            ...
            name = template.name
            stored = [pair.split(":", 1)[0] for pair in template.data.split(";") if ":" in pair]
            pairs = []
            for obj_name in stored:
                obj = scene.objects.get(obj_name)
                if obj:
                    pairs.append(f"{obj.name}:{obj.export_target_name or obj.name}")

        data = ";".join(pairs)

        # Overwrite if name already exists, otherwise add new
        template = next((t for t in scene.export_templates if t.name == name), None)
        if template is None:
            template = scene.export_templates.add()
            template.name = name
        template.data = data

        # Switch dropdown to the saved template
        for i, t in enumerate(scene.export_templates):
            if t.name == name:
                scene.export_active_template = str(i)
                break
...
class NOPPERS_OT_RemoveTemplate(Operator):
    ...
    def execute(self, context):
        scene = context.scene
        templates = scene.export_templates

        if not templates:
            return {'CANCELLED'}

        name = templates[int(scene.export_active_template)].name
        templates.remove(int(scene.export_active_template))
        scene.export_active_template = 'NEW'
...
class NOPPERS_OT_PrepareExport(Operator):
    ...
    def execute(self, context):
        ...
        idx = int(scene.export_active_template)
        template = scene.export_templates[idx]
        mappings = [pair.split(":", 1) for pair in template.data.split(";") if ":" in pair]
        if not mappings:
        ...
        temp_scene.source_scene_name = scene.name
        ...
        try:
            ...
            # Rename armatures, apply bone collection visibility
            for copy, target_name, src in copies:
                if src.type == 'ARMATURE':
                    copy.rename(target_name, mode='ALWAYS')
                    copy.data.rename(target_name, mode='ALWAYS')
                    enabled = {item.name for item in src.bone_collection_items if item.enabled}
                    for col in copy.data.collections:
                        col.is_visible = col.name in enabled
...
class NOPPERS_OT_ReturnToScene(Operator):
    ...
    def execute(self, context):
        ...
        source_name = staging_scene.source_scene_name
